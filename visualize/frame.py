...
class Frame(d6t.tasks.TaskCache):
    ...
    left_to_right = d6t.BoolParameter(default=False)  # attacking team always plays from left to right
    pitch_control = d6t.BoolParameter(default=False)

    def requires(self):
        reqs = {'tracking': self.clone(tr.LoadTrackingHalf)}
        if self.pitch_control == False:
            return reqs
        else:
    ...
    def run(self):
        ...
        if self.pitch_control:
            ...
            if ((frames['home_ball'].values[0] == 1 and self.left_to_right == False and closest_right_touchline == 'home') or
                    (frames['home_ball'].values[0] == 0 and self.left_to_right == False and closest_right_touchline == 'home')):
                PPCFa = PPCFa[:, ::-1, ::-1]
            ax.imshow(np.flipud(PPCFa[-1, :, :]), extent=(np.amin(xgrid), np.amax(xgrid), np.amin(ygrid), np.amax(ygrid)),
                      interpolation='hanning', vmin=0.0, vmax=1.0, cmap='bwr', alpha=0.625)
